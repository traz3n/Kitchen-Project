...
public class RecipeListUI extends JFrame {
    ...
    private static class RoundedPanel extends JPanel {
        private final Color bg; private final int arc;
        RoundedPanel(LayoutManager lm, Color bg, int arc) { super(lm); this.bg = bg; this.arc = arc; setOpaque(false); }
    ...
    private static class TableHeaderRenderer extends DefaultTableCellRenderer {
        ...
        TableHeaderRenderer(Color bg) { this.bg = bg; }
    ...
    private JComponent buildHeader() {
        ...
        r1.addActionListener(e -> { if (r1.isSelected()) loadRecipe(1); });
        r2.addActionListener(e -> { if (r2.isSelected()) loadRecipe(2); });

        JPanel left = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        left.setOpaque(false);
        left.add(r1); left.add(r2);
        ...
        header.getActionMap().put("r1", new AbstractAction(){ @Override public void actionPerformed(ActionEvent e){ r1.setSelected(true); r1.doClick(); }});
        header.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_2, mask), "r2");
        header.getActionMap().put("r2", new AbstractAction(){ @Override public void actionPerformed(ActionEvent e){ r2.setSelected(true); r2.doClick(); }});
    ...
    private JToggleButton segButton(String text) {
        ...
        if (text.endsWith("1")) b.setBackground(ACCENT); // initial
        return b;
    }
    ...
    private JPanel buildIngredientsCard() {
        ...
        rail.add(add); rail.add(update); rail.add(delete);
        ...
        p = field(); b = field(); c = field(); q = field(); e = field(); searchIng = field();
        form.add(label("Product"));  form.add(p);
        form.add(label("Brand"));    form.add(b);
        form.add(label("Category")); form.add(c);
        form.add(label("Quantity")); form.add(q);
        form.add(label("Expiry"));   form.add(e);
        form.add(label("Search"));   form.add(searchIng);
        ...
        searchIng.getDocument().addDocumentListener(new DocumentListener(){
            private void upd(){ sorter.setRowFilter(RowFilter.regexFilter(searchIng.getText(),0,1,2,4)); }
        ...
        ingredientsTable.getSelectionModel().addListSelectionListener(event -> {
            ...
            p.setText(it.getProduct()); b.setText(it.getBrand()); c.setText(it.getCategory());
            q.setText(String.valueOf(it.getQuantity())); e.setText(it.getExpiry());
        });

        add.addActionListener(e1 -> { Item it = fromFields(); if (it!=null) { ingredientsModel.add(it); autoPersistRecipe(); } });
        update.addActionListener(e12 -> {
            int r = ingredientsTable.getSelectedRow(); if (r<0) return;
            Item it = fromFields(); if (it!=null) { ingredientsModel.update(ingredientsTable.convertRowIndexToModel(r), it); autoPersistRecipe(); }
        });
        delete.addActionListener(e13 -> {
            int r = ingredientsTable.getSelectedRow(); if (r<0) { showInfo("Select a row to delete"); return; }
            ingredientsModel.remove(ingredientsTable.convertRowIndexToModel(r)); autoPersistRecipe();
        });
    ...
    private JPanel buildStepsCard() {
        ...
        form.setOpaque(false);
        stepField = field(); searchStep = field();
        form.add(label("Step"));   form.add(stepField);
        form.add(label("Search")); form.add(searchStep);
        card.add(form, BorderLayout.SOUTH);
        ...
        searchStep.getDocument().addDocumentListener(new DocumentListener(){
            private void upd(){ sorter.setRowFilter(RowFilter.regexFilter(searchStep.getText(),0)); }
            @Override public void insertUpdate(DocumentEvent e){upd();}
            @Override public void removeUpdate(DocumentEvent e){upd();}
            @Override public void changedUpdate(DocumentEvent e){upd();}
        });
        ...
        add.addActionListener(e1 -> {
            String s = stepField.getText().trim(); if (s.isEmpty()) return;
            stepsModel.addRow(new Object[]{s}); stepField.setText(""); autoPersistRecipe();
        });
        update.addActionListener(e12 -> {
            int r = stepsTable.getSelectedRow(); if (r<0) return;
            String s = stepField.getText().trim(); if (s.isEmpty()) return;
            stepsModel.setValueAt(s, stepsTable.convertRowIndexToModel(r), 0); stepField.setText(""); autoPersistRecipe();
        });
        delete.addActionListener(e13 -> {
            int r = stepsTable.getSelectedRow(); if (r<0) { showInfo("Select a row to delete"); return; }
            stepsModel.removeRow(stepsTable.convertRowIndexToModel(r)); autoPersistRecipe();
        });
    ...
    private JButton primaryBtn(String text) { return makeBaseBtn(text, ACCENT, Color.BLACK, ACCENT_2); }
    private JButton dangerBtn(String text)  { return makeBaseBtn(text, new Color(0xD65C5C), Color.WHITE, new Color(0xBF3F3F)); }
    ...
    private void styleTable(JTable table) {
        ...
        table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
                ...
                if (!isSelected) setBackground((row % 2 == 0) ? CARD_BG : STRIPE);
                return this;
            }
        });
    }
    ...
    private Item fromFields(){
        ...
        if (ps.isEmpty()) { showError("Invalid Input", "Product name is required", null); return null; }
        int qty; try { qty = Integer.parseInt(q.getText().trim()); } catch (Exception ex) { qty = 0; }
    ...
    private void loadRecipe(int id) {
        try {
            ...
            while (stepsModel.getRowCount() > 0) stepsModel.removeRow(0);
            for (String s : r.getSteps()) stepsModel.addRow(new Object[]{s});
        } catch (Exception ex) { showError("Load Error", ex.getMessage(), ex); }
    }
    ...
    private void autoPersistRecipe() {
        try {
            ...
            flashSaved();
        } catch (Exception ex) { showError("Save Error", ex.getMessage(), ex); }
    }
    ...
    private void installProportionalResize(JTable table, JScrollPane scroller, double[] weights, int[] minWidths) {
        final ComponentAdapter adapter = new ComponentAdapter() {
            private void resizeCols() {
                ...
                if (weights == null || weights.length != table.getColumnCount()) return;

                JViewport vp = scroller.getViewport(); if (vp == null) return;
                int w = vp.getWidth(); if (w <= 0) return;

                double sum = 0.0; for (double wt : weights) sum += wt; if (sum <= 0) return;

                TableColumnModel cm = table.getColumnModel();
                int used = 0; int[] target = new int[weights.length];

                for (int i = 0; i < weights.length - 1; i++) {
                    int mw = (minWidths != null && i < minWidths.length) ? minWidths[i] : 40;
                    int width = Math.max(mw, (int) Math.round(w * (weights[i] / sum)));
                    target[i] = width; used += width;
                }
                ...
                for (int i = 0; i < target.length; i++) {
                    ...
                    col.setMinWidth(Math.min(target[i], (minWidths != null && i < minWidths.length) ? minWidths[i] : 10));
                    col.setWidth(target[i]);
                }
                table.revalidate();
            }
            @Override public void componentResized(ComponentEvent e) { resizeCols(); }
            @Override public void componentShown(ComponentEvent e)   { resizeCols(); }
        };
    ...
    private void flashSaved() {
        ...
        toastTimer.schedule(new TimerTask() {
            @Override public void run() { SwingUtilities.invokeLater(() -> savedToast.setVisible(false)); }
        }, 900);
    }
